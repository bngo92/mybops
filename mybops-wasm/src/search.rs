use crate::{bootstrap::Collapse, plot::DataView};
use web_sys::{HtmlSelectElement, KeyboardEvent};
use yew::{Callback, Html, Properties, function_component, html, use_node_ref, use_state};
...
#[function_component]
pub fn Search(SearchProps { logged_in }: &SearchProps) -> Html {
    let help_collapsed = use_state(|| *logged_in);
    let split_view = use_state(|| false);

    let toggle_help = {
        let help_collapsed = help_collapsed.clone();
        Callback::from(move |_| help_collapsed.set(*help_collapsed))
    };
    let onclick = {
        let split_view = split_view.clone();
        Callback::from(move |_| split_view.set(*split_view))
    };
    let button_text = if *split_view {
        "Single View"
    } else {
        "Split View"
    };
...
pub fn SearchPane() -> Html {
    ...
    let search = {
        ...
        Callback::from(move |_| {
            let input = search_ref.cast::<HtmlSelectElement>().unwrap().value();
            let query = query.clone();
            let error = error.clone();
            wasm_bindgen_futures::spawn_local(async move {
                match crate::find_items(&input).await {
                    ...
                    Err(e) => {
                        error.set(Some(e.as_string().unwrap()));
                    }
                }
            });
        })
    ...
    let onchange = {
        ...
        let view = view.clone();
        Callback::from(move |_| {
            let v = select_ref.cast::<HtmlSelectElement>().unwrap().value();
            view.set(match &*v {
    ...
    if let Some(ref df) = *query
        && let Err(e) = view.draw(df)
    {
    ...
    let (class, error) = if let Some(ref error) = *error {
        (
    ...
    html! {
        <div class="row w-100">
            <div class="col-auto">
                <select ref={select_ref.clone()} class="form-select mb-3" {onchange}>
            ...
            <form {onkeydown}>
                <div class="d-flex gap-2">
                    <div class="flex-grow-1">
                        ...
                        <input ref={search_ref.clone()} type="text" {class} style="padding: .5rem 1rem; font-size: .875rem; border-width: 1px; min-width: 100%" placeholder={default_search}/>
                        if let Some(error) = error {
                            {error}
                        }
                    </div>
                    <button type="button" class="btn btn-success" onclick={search.clone()} style="height: fit-content">{"Search"}</button>
                    <button type="button" class="btn btn-success" onclick={create.clone()} style="height: fit-content" disabled={query.is_none()}>{"Create List"}</button>
                </div>
            </form>
